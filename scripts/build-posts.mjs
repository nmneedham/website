...
const OUTPUT_POSTS_DIR = path.join(BLOG_DIR, "posts");
...
const SITE = {
  ...
  blurb:
    "Hello, I'm Nick, a student in software development and electronics enthusiast. I enjoy programming and learning new things about electronics. I'm currently learning the Java programming language in school, so this is a blog to follow me on my journey and other projects.",
};
  ...
  return `
    <aside class="sidebar">
      <div class="sidebar-top">
        <a href="/" class="site-title">Nick Needham</a>
      ...
      <div class="sidebar-bottom">
        ...
        <div class="social-links">
          ...
          <a href="/blog/">blog</a>
          <a href="mailto:you@n.needham@live.com">email</a>
        </div>
      </div>
    </aside>`;
}
      ...
      (post) => `
          <article class="post-card">
            <a href="${post.href}">
              ...
              <p class="post-meta">${escapeHtml(post.date)} · ${escapeHtml(post.read)}</p>
    ...
    <main class="main">
      <section class="section">
        <header class="section-header">
          <h2>blog</h2>
          <span class="section-sub">generated from markdown</span>
        </header>

        <div class="post-list">
${cards || `<p class="post-excerpt">No posts yet.</p>`}
        </div>
      </section>
    </main>
  </div>
...
function buildPostHtml(post) {
  return `<!DOCTYPE html>
<html lang="en">
${pageHead(`${post.title} — Nick Needham`)}
<body>
  <div class="page">
    ${sidebar()}
      ...
      <div class="post-wrapper">
        <article class="post-article">
          <header class="post-header">
            <div class="post-kicker">${escapeHtml(post.tags[0] || "blog")}</div>
            <h1 class="post-title-main">${escapeHtml(post.title)}</h1>
            <div class="post-meta-line">
              <span>${escapeHtml(post.date)}</span> · <span>${escapeHtml(post.read)}</span>
            </div>
          </header>
...
async function main() {
  ...
  for (const filename of entries) {
    ...
    const date = formatDate(parsed.data.date || "1970-01-01");
    ...
    await fs.writeFile(
      path.join(OUTPUT_POSTS_DIR, `${slug}.html`),
      buildPostHtml(post),
      "utf8"
    );
  }
  ...
  await fs.writeFile(path.join(BLOG_DIR, "index.html"), buildBlogIndex(posts), "utf8");

  await fs.writeFile(
    POSTS_DATA_PATH,
    `export const POSTS = ${JSON.stringify(
      posts.map(({ slug, title, date, excerpt, tags, read, href }) => ({
        slug,
        title,
        date,
        excerpt,
